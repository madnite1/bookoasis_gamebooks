...
* **전 기종 7z ➔ ZIP 영구 자동 변환**: 롬 업로드, 재스캔, 풀스캔 시 모든 콘솔 및 아케이드의 `.7z` 롬셋을 에뮬레이터 100% 호환 표준 `.zip` 규격으로 즉시 무손실 자동 변환하고 네오지오 기판 칩셋을 자동 병합합니다.

### 5. 🩺 ROM 라이브러리 전수 무결성 진단 (Health Check, 관리자 전용)
* **풀스캔 옆 `[무결성 진단]` 버튼**: 관리자(Admin) 로그인 시에만 풀스캔 버튼 옆에 노출됩니다.
* **1,140+ 롬셋 전수 자동 진단**: 27,000여 개 DAT DB를 바탕으로 보유 중인 모든 아케이드/콘솔 롬의 내부 CRC32를 비교 분석하여 **100% 정상 구동 가능 게임**, **칩셋 누락 불완전 롬셋**, **CHD 디스크 필요 롬셋**, **현재 EmulatorJS/MAME2003 코어에서 게임별 호환성 제한으로 구동 불가한 롬셋**을 실시간 분류 리포트로 제공합니다.
* **불완전 롬셋 상세 사유 안내**: 구형 덤프여서 부족한 칩셋의 실제 파일명(예: `ddp3_bios.u37`, `bios.u42`)을 테이블 형태로 검색 및 확인할 수 있습니다.
* **게임별 코어 호환성 진단**: 내장 `mame_compatibility.db`를 이용해 `game not working`으로 알려진 MAME2003/MAME2003-Plus 게임을 파일 손상과 구분하여 `코어 미지원` 상태로 표시합니다.

### 6. 👥 유저별 완벽한 데이터 격리 (`plugins/data/bookoasis_gamebooks/`)
...
### 8. 🛡️ 관리자(Admin) 권한 제어
* 관리자(Admin) 권한을 가진 계정으로 로그인 시에만 **`[ROM 업로드]`, `[바이오스 관리]`, `[홈브류 허브]`, `[풀 스캔]`, `[무결성 진단]`, `[⚙️ 설정]`** 메뉴가 노출됩니다.
